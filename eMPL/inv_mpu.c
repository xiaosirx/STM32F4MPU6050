 ...
 * min(int a, int b)
 */
#if defined MOTION_DRIVER_TARGET_STM32F4
#include "stm32f4xx.h"
#include "iic.h"
#include "mydelay.h"
...
int mpu_init(void)
{
    ...
    /* Reset device. */
    data[0] = BIT_RESET;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))		//复位mpu
        return -1;
    delay_ms(100);

    /* Wake up chip. */
    data[0] = 0x00;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))		//唤醒mpu
        return -1;

#if defined MPU6050
    /* Check product revision. */
    if (i2c_read(st.hw->addr, st.reg->accel_offs, 6, data))			//检查版本
        return -1;
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    /* Set to invalid values to ensure no I2C writes are skipped. */	//设置无效值以确保iic写功能不被跳过
...
#endif
    /* mpu_set_sensors always preserves this setting. */				//Mpu_set_sensors总是保持这个设置
    ...
    if (mpu_set_gyro_fsr(2000))				//陀螺仪满量程±2000dps
        return -1;
    if (mpu_set_accel_fsr(2))				//加速度计满量程±2g
        return -1;
    if (mpu_set_lpf(42))
        return -1;
    ...
    if (mpu_set_compass_sample_rate(10))
        return -1;
#else
    /* Already disabled by setup_compass. */		//已经被setup_compass禁用
    if (mpu_set_bypass(0))
        return -1;
#endif
